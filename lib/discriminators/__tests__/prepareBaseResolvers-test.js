...
describe('prepareBaseResolvers()', () => {
  describe('setDKeyEnumOnITCArgs()', () => {
    ...
    it('should set DKey field type to DKeyETC on record args', () => {
      for (const resolver of resolversWithRecordArgs) {
        ...
        if (resolver.name === 'createOne') {
          expect(resolver.getArgTC(interestArgs[1]).getFieldConfig(DKeyFieldName).type).toEqual(graphql.GraphQLNonNull(DKeyETC.getType()));
        } else {
    ...
    it('should set DKey field type to DKeyETC on records args', () => {
      for (const resolver of resolversWithRecordsArgs) {
        ...
        expect(resolver.getArgTC(interestArgs[2]).getFieldConfig(DKeyFieldName).type).toEqual(graphql.GraphQLNonNull(DKeyETC.getType()));
      }
    });
  });
  ...
  describe('createMany: Resolver', () => {
    ...
    it('should set resolver records field type to NonNull Plural DInterface', () => {
      expect(resolver.getTypeComposer().getFieldType('records')).toEqual(new graphql.GraphQLNonNull(graphql.GraphQLList(DInterfaceTC.getType())));
    });
  });
  describe('findById: Resolver', () => {
    ...
    it('should set resolver type to DInterface List', () => {
      expect(resolver.getType()).toEqual(graphql.GraphQLList(CharacterDTC.getDInterface().getType()));
    });
  });
  describe('findMany: Resolver', () => {
    it('should set resolver type to DInterface List', () => {
      expect(CharacterDTC.getResolver('findMany').getType()).toEqual(graphql.GraphQLList(DInterfaceTC.getType()));
    });
  });
  ...
  it('should set DKey field type to NonNull(DKeyETC) on record arg, createOne', () => {
    expect(CharacterDTC.getResolver('createOne').getArgTC('record').getFieldType(CharacterDTC.getDKey())).toEqual(graphql.GraphQLNonNull(CharacterDTC.getDKeyETC().getType()));
  });
  it('should set type on items in pagination resolver to DInterface List, pagination', () => {
    expect(CharacterDTC.getResolver('pagination').getTypeComposer().getFieldType('items')).toEqual(graphql.GraphQLList(CharacterDTC.getDInterface().getType()));
  });
