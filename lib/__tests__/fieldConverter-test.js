...
import { deriveComplexType, getFieldsFromModel, convertFieldToGraphQL, ComplexTypes, scalarToGraphQL, arrayToGraphQL, embeddedToGraphQL, enumToGraphQL, documentArrayToGraphQL, referenceToGraphQL } from '../fieldsConverter';
...
describe('fieldConverter', () => {
  ...
  describe('convertFieldToGraphQL()', () => {
    ...
    it('should use existed GraphQLMongoID in schemaComposer', () => {
      schemaComposer.clear();
      expect(schemaComposer.has('MongoID')).toBeFalsy();
      schemaComposer.set('MongoID', 'MockGraphQLType');
      const mongooseField = {
        path: 'strFieldName',
        instance: 'ObjectID'
      };
